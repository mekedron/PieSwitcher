...
/// reveal is in flight so that if Bringr is killed mid-reveal (a crash, a force
...
/// snapshot is trivially `Codable`. The other apps keep their pids across a Bringr
/// crash (only Bringr died, not them), so the ids still resolve on the next launch.
